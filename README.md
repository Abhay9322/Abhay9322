# 🚀 Welcome to My Full Stack Web Development Journey!

### 👨‍💻 MERN Stack Developer | 🌐 Full Stack Enthusiast 

Hello! I'm **Abhay Suryawanshi**, a passionate **Full Stack Web Developer** currently pursuing a B.Tech in Computer Engineering at **Government College of Engineering, Yavatmal**. I enjoy building full-stack web applications that solve real-world problems and offer great user experiences.

---

## 🔧 Tech Stack

**Frontend:**
- React.js ⚛️
- HTML5 & CSS3 🌐
- JavaScript (ES6+) 💡
- Tailwind CSS & Bootstrap 🎨

**Backend:**
- Node.js 🌱
- Express.js 🚂
- MongoDB 🟢 (Mongoose ORM)
- REST APIs

**Tools & Others:**
- Git & GitHub 🔧
- Postman 📮
- VS Code 🖥️
- Vite ⚡

---

## 🛠️ Projects

### 🍽️ Food Ordering Web App (MERN Stack)
A full-stack application where users can browse food items, add to cart, and place orders online.  
**Features:** Login/Signup, Dynamic Menu, Cart Functionality, Order Placement  
**Tech Used:** React, Node.js, Express, MongoDB  
🔗 [GitHub Repo](https://github.com/Abhay9322/Food-Ordering-MERN)

---

### 🛍️ E-commerce Product Page
An interactive product page UI with add-to-cart feature and dynamic product rendering.  
**Concepts:** Props, State Management, Routing  
🔗 [GitHub Repo](https://github.com/Abhay9322/E-commerce-Product-Page)

---

### 📚 Quiz App with Timer
A React-based quiz app with a countdown timer, score tracking, and question navigation.  
**Concepts:** useEffect, useState, conditional rendering  
🔗 [GitHub Repo](https://github.com/Abhay9322/Quiz-App)

---

## 🎓 Education

- **B.Tech in Computer Engineering**  
  *GCOE Yavatmal (Final Year)*

- **HSC (12th):** 92.33%  
  *Yashwant Mahavidyalaya, Nanded*

- **SSC (10th):** 80%  
  *Z.P. High School Shewala, Hingoli*

---

## 📫 Let's Connect!

- 🔗 [LinkedIn](https://www.linkedin.com/in/abhay-suryawanshi-a0b9632b1)
- 💻 [GitHub](https://github.com/Abhay9322)
- 📧 Email: abhaysuryawanshi9322@gmail.com

---

> 💡 *“Code is like humor. When you have to explain it, it’s bad.” – Cory House*  
> Keep learning. Keep building. 💻
